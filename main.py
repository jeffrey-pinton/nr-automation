...
def reconcile(df, verbose=False):
    ...
    while CURRENT_DATE > due_date:
        ...
        for tr_date in transactions.TR_DATE.unique():
            ...
            # compute penalty
            if (temp_pd_digit > 0) and ((tr_date - due_date_old).days > 5) and not is_bayanihan:
                print(tr_date)
                print(due_date_old)
                penalty_start_date = add_months(due_date, -temp_pd_digit)
                penalty = compute_penalty(tr_date, penalty_start_date, ma)
                df.loc[df.TR_DATE == tr_date, 'recomputed_penalty'] = penalty
            else:
                penalty = 0
...
def get_account_details(text):
    import re
    acct_no = re.search(r"nr (\w+)", text)[1]
    df = get_account(acct_no)
    df = format_date(df)
    df, pd_string = reconcile(df, verbose=False)
    field_include = ['TR_DATE', 'PARTICULAR', 'DEBIT', 'CREDIT', 'BALANCE', 'PD Char', 'recomputed_total_balance']

    message = f'Account Name: {df.ACCT_NAME.unique()[0]}' \
              f'Account Number: {df.ACCT_NO.unique()[0]}' \
              f'MA: {df.ma.unique()[0]}\n' \
              f'FDD: {df.FDD.unique()[0]}\n' \
              f'LDD: {df.LDD.unique()[0]}\n' \
              f'Repo: {True in df.REPO.unique()}' \
              f'PD String: {pd_string}'
    # return alt_response(df[field_include], message)


def sample():
    # df = pd.read_csv('data/219_quibo_sample.csv')
    # df = format_date(df, cols=('FDD', 'TR_DATE'), date_format='%d/%m/%Y')
    # df['LDD'] = add_months(df.FDD.unique()[0], df.Term.unique()[0] - 1)
    acct_no = '00561289'
    df = get_account(acct_no)
    df = format_date(df)
    ...
    field_include = ['TR_DATE', 'PARTICULAR', 'REBATE', 'DEBIT', 'CREDIT', 'BALANCE', 'PD Char', 'recomputed_total_balance']
    ...
    message = f'Account Name: {df.ACCT_NAME.unique()[0]}\n' \
              ...
              f'Repo: {True in df.REPO.unique()}\n' \
              f'PD String: {pd_string} \n\n' \
              f'UNRECOGNIZED REBATE/S: {unrecognized_rebate} \n' \
              f'REV OF EARNED S: {rev_earned_s}'

    final_df = df[field_include]
